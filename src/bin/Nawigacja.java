...
//            dodajDoListyLokalnychVertex(new bin.Vertex(listy.getListaBudynkow().get(i).getNazwa(),
        ...
        for (int i = 0; i < listaVertex.size(); i++) {
            ...
            for (Vertex next : col) {
                if (listaVertex.get(i).x == next.x) {
                    ...
                    //    tempArr.add(tempArr.size(), new bin.Edge(this.mojaPozycja, Math.abs(listaVertex.get(i).y - this.mojaPozycja.y)));
                } else {
                    tempArr.add(tempArr.size(), new Edge(next, Math.abs(listaVertex.get(i).x - next.x)));
                    //  tempArr.add(tempArr.size(), new bin.Edge(this.mojaPozycja, Math.abs(listaVertex.get(i).x - this.mojaPozycja.x)));
                }
            }
