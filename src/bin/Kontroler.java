...
public class Kontroler {
    ...
    public void main(Controller controller) {
        ...
        listy.setListaCustomVertex(new ArrayList<>(Arrays.asList(new Vertex("A", 0, 0),
                new Vertex("B", 0, 21), new Vertex("C", 11, 21), new Vertex("D", 0, 30),
                new Vertex("E", 21, 16), new Vertex("F", 21, 30), new Vertex("G", 16, 7),
                new Vertex("H", 11, 7), new Vertex("I", 16, 0), new Vertex("J", 21, 7),
                new Vertex("K", 27, 16), new Vertex("L", 27, 0)
...
//        utworzPojazd(listy, "MAN", baza);
    ...
    private void utworzPojazd(Listy listy, String nazwa, Baza baza) {
        Pojazd pojazd = new Pojazd(listy, nazwa, baza);
        pojazd.start();
        listy.getListaPojazdow().add(listy.getListaPojazdow().size(), pojazd);
    }

    private void utworzBudynek(Budynek budynek, Listy listy) {
        if (!budynek.getClass().getCanonicalName().equalsIgnoreCase("bin.Magazyn"))
            if (!budynek.getClass().getCanonicalName().equalsIgnoreCase("bin.Stacja"))
                new Thread((Runnable) budynek).start();
        String toPass = budynek.getClass().getCanonicalName().substring(4);
        System.out.println(toPass);
        controller.utworzBudynek(budynek.getX(), budynek.getY(), budynek.getNazwa(), toPass);
